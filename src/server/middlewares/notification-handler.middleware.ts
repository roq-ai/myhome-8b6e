...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'maintenance_request_create.create': {
    roles: ['property-owner', 'property-manager', 'maintenance-staff'],
    key: 'maintenance_request',
    tenantPath: [],
    userPath: [],
  },
  'lease_agreement_signed.create': {
    roles: ['property-owner', 'property-manager'],
    key: 'lease_agreement',
    tenantPath: [],
    userPath: [],
  },
  'rent_payment_made.create': {
    roles: ['property-owner', 'property-manager'],
    key: 'rent_payment',
    tenantPath: [],
    userPath: [],
...
const ownerRoles: string[] = ['property-owner'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['property-owner', 'property-manager', 'maintenance-staff', 'tenant', 'guest'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.property.findFirst({
