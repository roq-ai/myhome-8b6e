...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiHome } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['property-owner', 'property-manager', 'maintenance-staff', 'tenant', 'guest'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PropertyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/properties` : null),
    () => getProperties({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Properties',
      path: isTenantUser ? `/properties/view/${data?.[0]?.id}` : '/properties',
      entity: 'property',
